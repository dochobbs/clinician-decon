"""Compare OpenMed PHI-NER masking vs Haiku LLM-rewrite decon.
...
Two structurally different approaches, scored on the same axes:
  ...
  - Haiku: BRIGHTLINE_ROUTER_PROMPT → rewrite query as a clinical
...
Both produce a "PHI-free version" of the query. We score:
  ...
  - JSON validity (Haiku only — OpenMed's output is plain text)
  - Latency (model load excluded)
  - Output character length

Usage:
  python3 scripts/openmed_vs_haiku_decon.py
  python3 scripts/openmed_vs_haiku_decon.py --skip-haiku  # NER only
  python3 scripts/openmed_vs_haiku_decon.py --model OpenMed/...
"""
...
def main():
  ...
  print()
  print(f"OpenMed vs Haiku decon  |  n={len(TEST_QUERIES)}")
  print("=" * 80)
  ...
  for tq in TEST_QUERIES:
    ...
    if not args.skip_haiku:
      try:
        ...
        print(f"  haiku    {h['latency_s']*1000:6.0f}ms  "
  ...
  if not args.skip_haiku:
    h_rows = [r for r in results if r.get("haiku") and not r["haiku"].get("error")]
    if h_rows:
      h_total_leaks = sum(
        len(r["haiku"].get("leaks_in_cds", [])) +
        len(r["haiku"].get("leaks_in_chart", []))
        for r in h_rows)
      h_avg_lat_ms = sum(r["haiku"]["latency_s"] for r in h_rows) / len(h_rows) * 1000
      h_valid = sum(1 for r in h_rows if r["haiku"].get("parsed") is not None)
      print(f"  haiku    n={len(h_rows):2d}  valid_json={h_valid}/{len(h_rows)}  "
            f"leaks={h_total_leaks}  avg_lat={h_avg_lat_ms:.0f}ms")

  Path(OUT_DIR).mkdir(parents=True, exist_ok=True)
  ts = datetime.now().strftime("%Y%m%d_%H%M%S")
  out = f"{OUT_DIR}/openmed_vs_haiku_{ts}.json"
