"""Decontextualization parity test: gemma4:e4b vs Haiku.
...
Usage:
  ...
  python3 scripts/decon_parity_test.py --skip-haiku  # gemma only
"""
...
def haiku_route(query: str, age: int | None, sex: str | None) -> dict:
...
def main():
  ...
  print(f"Decon parity test: gemma={args.gemma_model} vs haiku={HAIKU_MODEL}")
  ...
  for tq in TEST_QUERIES:
    ...
    if not args.skip_haiku:
      try:
        ...
        print(f"  haiku:    {h['latency_s']:5.2f}s  leaks={leaks}  needs_cds={needs}")
        if cds: print(f"            cds: {cds[:90]}{'…' if cds and len(cds)>90 else ''}")
        row["haiku"] = h
      except Exception as e:
  ...
  with open(out, "w") as fh:
    json.dump({"timestamp": ts, "gemma_model": args.gemma_model,
               "haiku_model": HAIKU_MODEL, "results": results}, fh, indent=2)
  print(f"\n→ {out}")
