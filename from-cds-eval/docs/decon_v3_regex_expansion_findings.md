...
**Test:** n=1132 / 2526 PHI terms (synthetic seed=42 + seed=43 + Amboss adversarial)
...
| Amboss R1 basic | 36% | **25%** | -11pp |
| Amboss R2 adversarial | 34% | **27%** | -7pp |
| Amboss R3 max-adv | 6% | **6%** | flat |
| Amboss R4 prod-rep n=95 | 6% | **6%** | flat |
...
This is a design difference vs Haiku decon:

| Approach | Method | Result |
|---|---|---|
| **Haiku** | Generative rewrite — produces a clean clinical question with no patient context at all | 100% strip rate; clinical values not preserved |
| **OpenMed regex+NER** | Token-classification + regex; masks identifiers only | ~94% on Amboss (counts clinical values as PHI); preserves clinical context |
...
If we filter the Amboss "PHI" annotations to HIPAA Safe Harbor identifiers
...
| Amboss R3 max-adv | 0% (the 6% was clinical content) |
| Amboss R4 prod-rep | <1% (most "leaks" are clinical values) |
| Amboss R1+R2 (subtle/implicit) | 5-10% — implicit references still slip |
...
**The 6% Amboss R4 number should be reported with context** — it's a
