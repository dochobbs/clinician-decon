# OpenMed PHI-NER vs Haiku Decon — Parity Test Results

**Date:** 2026-05-10
**Test:** `scripts/openmed_vs_haiku_decon.py`
**OpenMed model:** `OpenMed/OpenMed-PII-SuperClinical-Large-434M-v1` (CPU)
**Haiku model:** `claude-haiku-4-5-20251001` (API)
**N:** 10 synthetic-PHI clinical queries
**Result file:** `results/decon_parity/openmed_vs_haiku_20260510_085527.json`

---

## Headline

| Metric | OpenMed (NER mask) | Haiku (LLM rewrite) |
|---|---|---|
...
repo. Haiku stays available as an optional fallback for edge cases.
...
Haiku:    rewrote cleanly into a clinical query, no leak
```
...
sent to Exa would be a real HIPAA event.
...
| Q | Query (truncated) | OpenMed spans | OpenMed leaks | Haiku result |
|---|---|---|---|---|
...
[Layer 3 (optional): Confidence gate]
   - If NER confidence < 0.85 on any span,
     ...
     numerical groups, fall back to Haiku for a
     creative rewrite
...
with Anthropic credentials who want belt-and-suspenders safety.

---

## What this changes for the architecture

- ✅ OpenMed PHI-NER **does work** as a Haiku decon replacement, with caveats.
...
- ✅ Latency budget for decon stays well under 1 second even on CPU (Layer 1
  + Layer 2 = ~810 ms total, vs Haiku's 1641 ms over network).
