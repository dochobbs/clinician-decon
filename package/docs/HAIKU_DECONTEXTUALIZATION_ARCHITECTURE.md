# Haiku Decontextualization Architecture
...
Clinical decision support requires web search for guideline freshness. Web search APIs (Exa.ai, Google Custom Search, Brave) are not HIPAA-covered. Physician queries may contain patient-identifiable information.

**Example:** A physician types:
> "Marcus Johnson, DOB 3/15/2013, MRN LP-2024-08432, needs his 12-year-old vaccines per AAP"

Sending this to a search API is a HIPAA violation. But the search engine only needs:
...
└───────────┬───────────┘             │
            ...
            ▼                         ▼
┌─────────────────────────────────────────────────────────┐
│ Claude Sonnet (BAA-covered)                             │
│                                                         │
│ Receives:                                               │
│   - ws2 prompt (system)                                 │
│   - Exa search results (system, appended)               │
...
| Data | Haiku (Layer 1) | Regex (Layer 2) | Exa Search | Sonnet |
|------|:---------------:|:---------------:|:----------:|:------:|
...
| Exa results | — | — | Returns | Sees (BAA) |

---

## Layer 1: Haiku Decontextualization
...
The prompt was designed for HIPAA compliance, but the "append current guidelines 2025 2026" instruction also serves a quality purpose. It tells Exa to prioritize recent guideline documents in search results.

**Evidence:** B_auto (raw query → Exa, no Haiku) scored 1.5/2 on freshness. Haiku+Exa scored 2.0/2 on the same queries. The freshness terms Haiku appends are the difference.

Queries where this mattered:
- F03 (T2DM first-line): B_auto found ADA 2024, Haiku+Exa found ADA 2026
- F09 (male UTI): B_auto cited IDSA 2010, Haiku+Exa cited IDSA 2025
...
Safety net. If Haiku fails to strip a patient identifier, the regex layer catches it before the query reaches Exa.
...
1. **Option A:** Skip Exa search, generate CDS answer without web context (quality drops from ~89 to ~85 — still functional)
2. **Option B:** Retry Haiku with a stricter prompt: "Your previous output contained patient identifiers. Return ONLY the clinical topic."
...
The `clinical_question` field is what the physician typed. The `patient_context` is structured data extracted from the EHR chart by the integration layer. Only `clinical_question` enters the Haiku decontextualization pipeline.

### Why This Matters

Even without Haiku, the field separation means:
...
The risk vector is the physician typing identifiers into the question field. Haiku handles that. But the field separation reduces the attack surface to just one input.
...
*1 regex false positive ("male" flagged as PHI). Zero real Haiku failures.
...
Haiku's decontextualization behavior may change with model updates. When `claude-haiku-4-5` is superseded:
...
- Haiku output length (unusually long outputs may indicate format drift)
