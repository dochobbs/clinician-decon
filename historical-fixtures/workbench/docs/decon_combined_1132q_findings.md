...
| historical project R1 (basic) | 15 | 36 | 13 | 36% | Subtle clinical context |
| historical project R2 (adversarial) | 12 | 41 | 14 | 34% | Multi-language, implicit refs |
| historical project R3 (max adv) | 10 | 31 | 2 | 6% | Encoding attacks |
| historical project R4 (production-rep) | 95 | 821 | 50 | 6% | Realistic clinician queries |
| **Combined** | **1132** | **2526** | **79** | **3.1%** | |

**This is an inverse failure profile from cloud rewrite model decon.** cloud rewrite model passed 100%
...
| Adversarial category | cloud rewrite model | OpenMed regex+NER | Why |
|---|---|---|---|
...
| **Practice/insurance names** | "Lakes Pediatrics", "insurer", "Children's" | Not in regex; NER doesn't reliably tag organizations |
...
### historical project R1 — basic PHI, 7 of 15 queries leaked
...
PHI11 insurance_reference             ✗ 1 leak   (insurer)
...
### historical project R2 — adversarial, 9 of 12 leaked
...
### historical project R3 — max adversarial, 9 of 10 PASS
...
chain-of-thought exposure case. Better than cloud rewrite model's 10/10 by a different
mechanism: we're not vulnerable to the attack vector, just to the
implicit-reference vector.

### historical project R4 — production-representative, 35 of 95 leaked at 6%
...
The LLM reviewer can be local (gemma4:e4b at 600ms-1s) or cloud (cloud rewrite model).
...
- Average latency: ~600ms — still well under cloud rewrite model's 1641ms-over-network

This recovers cloud rewrite model-level coverage on the hard cases without giving up
...
(0% on canonical patterns, 94% on production-rep queries). The historical project
team's adversarial work showed that cloud rewrite model scored 99%+; we're at 97% by
...
Recovers cloud rewrite model-level coverage at half the latency on average.
