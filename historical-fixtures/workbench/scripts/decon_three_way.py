"""Three-way decon comparison: OpenMed (NER+regex) vs gemma4:e4b vs cloud rewrite.
  ...
  - **cloud rewrite** is the cloud reference (cloud API). Skipped by default
    when --skip-cloud-rewrite is set.
...
Scoring:
  ...
  - Latency
  - For LLM rewriters (gemma + cloud rewrite): JSON validity + needs_cds match
"""
...
from scripts.decon_parity_test import (  # noqa: E402
  TEST_QUERIES, gemma_route, cloud_rewrite_route, build_user_prompt, _parse_json,
)
...
def main():
  ...
  ap.add_argument("--skip-cloud-rewrite", action="store_true",
                  help="Skip the cloud rewrite cloud comparison (fully local mode)")
  args = ap.parse_args()
  ...
  print(f"  cloud_rewrite:   {'skipped' if args.skip_cloud_rewrite else 'cloud reference'}")
  ...
  for tq in TEST_QUERIES:
    ...
    if not args.skip_cloud_rewrite:
      try:
        ...
        print(f"  cloud_rewrite    {h['latency_s']*1000:6.0f}ms  valid_json={h.get('parsed') is not None}  "
              f"leaks={len(h_leaks)}")
        if cds:
          print(f"           cds: {cds[:90]}{'…' if len(cds)>90 else ''}")
        h["leaks"] = h_leaks
  ...
  def summarize(name: str, key: str, leak_field: str = "leaks"):
    ...
    extra = ""
    if name in ("gemma", "cloud_rewrite"):
