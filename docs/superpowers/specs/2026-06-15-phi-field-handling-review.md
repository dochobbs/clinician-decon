...
- DOB: removed as a date, but converted to a clinical age band when parseable.
- Exact ages: generalized to bands, e.g. `52yo` to `adult 45-64`,
  `11-year-old` to `early adolescent`, and `18 months` to `toddler 12-23 months`.
- Age 90 or older: normalized to `adult age 90 or older`.
...
- HPV search can preserve the clinical threshold without exact age:
  `early adolescent in HPV/Tdap vaccine range HPV vaccine schedule current guidelines`.
- Explicit ages normalize to bands and age 90+ is aggregated.
...
- The UI should expose age handling as a safety feature: age band by default, task-specific
  threshold hints for vaccines/screening/dosing, and no exact age in V1 external prompts.
