# External De-ID Baseline Representative Error Samples

Date: 2026-06-16

These are synthetic test cases from the Clinician Decon validation suites. They are representative
examples from the external de-ID baseline head-to-head run, scored against our clinician LLM handoff rubric:
remove PHI while preserving or safely generalizing clinically necessary facts.

The external de-ID baseline was run in `asterisk` mode with its default rule config after a local Python 3.13+
compatibility shim for old inline `(?i)` regex flags.

## Summary

| Error class | Sample count |
| --- | ---: |
...
External de-ID baseline output:
...
```text
Alias: Blue. Legal name Rowan Smith; insurance policy insurer TXQ-993812.
...
- external de-ID baseline is a classic note scrubber.
- Clinician Decon is a prompt decontextualizer.
- For LLM handoff, we need safe transformation, not only masking.

The external de-ID baseline is still useful as a source of regex ideas and adversarial tests, but these examples are
